...
func main() {
	...
	categories := categories.NewCategoryHandler(categoriesRepo)

	// Set up routing
	mux := http.NewServeMux()
	mux.HandleFunc("GET /catalog", cat.HandleGet)
	mux.HandleFunc("GET /catalog/{code}", cat.HandleGetByCode)
	mux.HandleFunc("GET /categories", categories.HandleGet)
	mux.HandleFunc("POST /categories", categories.HandleCreate)
